...
class SuspiciousLoadingNotebookPage(BasePage):
   ...
   def checkbox_notebook_suspicious_approved(self, row_number):
       return self.page.locator(f"tr:nth-child({row_number}) td:nth-child(6) .mdc-checkbox__native-control").first

   def checkbox_notebook_suspicious_denied(self, row_number):
       return self.page.locator(f"tr:nth-child({row_number}) td:nth-child(7) .mdc-checkbox__native-control").first
