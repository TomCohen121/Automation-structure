...
@pytest.mark.permissions_development_worker
@allure.story("Permissions for Development Worker")
@allure.description("Checking Permissions for Development Worker")
def test_permissions_for_development_worker(f, add_allure_attach, page):
    #Dashboard
    f.functions.assert_element_exists(f.permissions.comp_personal_details(),"The Component - רכיב פרטים אישיים")
    f.functions.assert_element_not_exists(f.permissions.comp_general_info_for_session(),"The Component - נתונים כללים למועד")
    ...
    f.functions.assert_element_exists(f.permissions.btn_show_formats(), "The Button - הצג פורמטים in Suspicious Loading")
