...
def test_regular_loading_end_to_end(from_page, add_allure_attach, page):
   ...
   finished_text = page.query_selector("(//div[@class='finished'])[1]").inner_text()
   print(f"num of loading charged {finished_text}")
   ...
   # table_notebook_grade_after = from_page["Functions"].number_to_float(from_page["NotebookPage"].txt_table_notebook_grade(2))
