...
def test_private_distribution_for_role(f, add_allure_attach, page):
    f.workflow.send_message_to_recipient("רים אבו שריקי","im role header", "im role body")
    f.functions.go_to_user(UsersForMessages.role_user)
    f.messages.btn_first_message().click()
    f.workflow.assert_message_display_and_content("im role header", "im role body")