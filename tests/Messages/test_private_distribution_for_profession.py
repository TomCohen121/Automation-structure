...
def test_private_distribution_for_profession(f, add_allure_attach, page):
    f.workflow.send_message_to_recipient("איתן ישראלי","im profession header", "im profession body")
    f.functions.go_to_user(UsersForMessages.profession_user)
    f.messages.btn_first_message().click()
    f.workflow.assert_message_display_and_content("im profession header", "im profession body")
