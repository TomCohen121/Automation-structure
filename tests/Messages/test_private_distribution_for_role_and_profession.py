...
def test_private_distribution_for_role_and_profession(f, add_allure_attach, page):
    f.workflow.send_message_to_recipient("סימה פומרניץ" ,"im role and profession header", "im role and profession body")
    f.functions.go_to_user(UsersForMessages.role_and_profession_user)
    f.messages.btn_first_message().click()
    f.workflow.assert_message_display_and_content("im role and profession header", "im role and profession body")